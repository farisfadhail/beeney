<x-app-layout>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('admin.article.update', $article->id) }}" enctype="multipart/form-data" class="mt-8">
        @csrf
        @method('PUT')
        ...
        <div class="flex flex-col mt-8">
            ...
            <textarea name="article_body" id="editor">{{ old('body', $article->article_body) }}</textarea>
            @error('body')
                <small class="text-red-700">{{ $message }}</small>
            @enderror
            @error('user_id')
